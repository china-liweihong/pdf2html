...
$(function(){
    ...
    var bigimg = $(".container").css("height");
        bigimg = parseInt(bigimg);
		bigimg=1170;
	var smallimg = 188;
	//循环函数
	function autoplay(){
	...
	$(".scrollpic li").dblclick(function(){
		...
		//大图上移
		$(".focus .focuspic").animate({"marginLeft":slideclick*-bigimg},{duration:animate_time,queue:false});
		...
		if(slideclick<picnumber){
			...
			if(slideclick==0||slideclick==1){
				...
				$(".scrollpic ul").animate({"marginLeft":(slideclick-1)*-smallimg},{duration:animate_time,queue:false});//显示最后4张图片不在滚动
			}
		}

		//判断上下按钮是否可点击
		if(slideclick==0){
			$(".focus #prev").addClass("disabled");
			$(".focus #next").removeClass("disabled");
		}else if(slideclick==picnumber){
			$(".focus #prev").removeClass("disabled");
			$(".focus #next").addClass("disabled");
		}else{
			$(".focus #prev").removeClass("disabled");
			$(".focus #next").removeClass("disabled");
		}
	...
	$(".scrollpic li").mouseover(function(){		
		...
		$(".focuspic").animate({"marginLeft":slidebtn_hover_chi*bigimg-bigimg},{duration:animate_time,queue:false});
	...
	$(".focus #next").click(function(){
		...
		if(slideclick<=picnumber){
			...
			$(".scrollpic li").eq(slideclick).trigger("dblclick");
			$(".scrollpic ul").animate({"marginLeft":(slideclick-2)*-smallimg},{duration:animate_time,queue:false});// 缩略图最后滚动的左侧距离位置 正好4张缩略图显示
		}else{
			$(".scrollpic li").eq(0).trigger("dblclick");
			slideclick = 0
		}
